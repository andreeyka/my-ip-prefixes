...
def load_exclude_cidrs(names: list[str]) -> tuple[list[str], dict[str, int]]:
    """Скачивает .srs файлы провайдеров/категорий для исключения из CIS.

    Файлы берутся из корня репо: {name}/{name}.srs
    Возвращает (список CIDR, словарь name → кол-во префиксов в источнике).
    """
    all_cidrs: list[str] = []
    stats: dict[str, int] = {}
            ...
            for rule in data.get("rules", []):
                ...
                if cidrs:
                    print(f"  Исключение {name}: {len(cidrs)} префиксов",
                          file=sys.stderr)
                    all_cidrs.extend(cidrs)
                    stats[name] = stats.get(name, 0) + len(cidrs)
...
def generate_readme(
    ...
    leaked_pct: float,
    reference_table: str,
    exclude_stats: dict[str, int] | None = None,
    excluded_ips: int = 0,
) -> str:
...
def main() -> None:
    ...
    if exclude_names:
        ...
        exclude_cidrs, exclude_stats = load_exclude_cidrs(exclude_names)
        if exclude_cidrs:
            ips_before = sum(
                IPv4Network(c, strict=False).num_addresses
                for c in all_cidrs if "." in c
            )
            ...
            excluded_ips = ips_before - ips_after
            print(f"Исключено из CIS: {excluded_ips:,} IP", file=sys.stderr)
    ...
    readme = generate_readme(
        ...
        exclude_stats=exclude_stats or None,
